...
void 삼국시대(EventManager& evt_mng) {
	...
	{
		EventMaker e("삼국통일", EventPriority::Highest);
		e->content =
...
군사력++";
...

	{
		...
		{
			EventSelectItemMaker s(e, "조세가 없다는 국가는 유지 될수 없지.");
			s->content =
				"식량++";
			s->Effect = [&](Gamedata& data) {
	...
	}
	{
		...
		e->content =
			"가뭄입니다. 백성들이 굶주리고 있습니다. 부디 자비롭게 대처하십시요";
...

	{
		...
		e->CanStart = [&](Gamedata& data) {
			return data.dict["시대"] != "현대시대" && data.dict["시대"] != "조선시대" && evt_mng.RandomInt(0, 9) == 0 && data.dict["호족의 불복"] == "";
		};
	...
	}
	{
		...
		e->CanStart = [&](Gamedata& data) {
			return data.dict["시대"] != "현대시대" && data.dict["시대"] != "조선시대" && data.dict["호족의 불복"] == "시작";
		};
	...
	}
	{
		...
		e->CanStart = [&](Gamedata& data) {
			return data.dict["시대"] != "현대시대" && data.dict["시대"] != "조선시대" && data.dict["호족의 불복"] == "반란";
		};
