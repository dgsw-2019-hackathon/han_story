...
신앙-";
	...
	}
	{
		...
		e->CanStart = [&](Gamedata& data) {
			return evt_mng.RandomInt(0, 4) == 0 && data.dict["시대"] == "조선시대";
		};
	...
	}
	{
		...
		e->CanStart = [&](Gamedata& data) {
			return evt_mng.RandomInt(0, 6) == 0 && data.dict["시대"] == "조선시대";
		};
...
우리는 이를 막기위해 대책을 세워야 합니다.";
		...
		e->CanStart = [&](Gamedata& data) {
			if (data.dict["임진왜란"] == "" && evt_mng.RandomInt(0, 6) == 0 && data.dict["시대"] == "조선시대")
			{
				data.dict["임진왜란"] = "시작됨";
				return true;
			}
